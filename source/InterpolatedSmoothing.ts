...
import {GoodTuringSmoothing} from "./GoodTuringSmoothing";
import {NGram} from "./NGram";
import {KFoldCrossValidation} from "nlptoolkit-sampling/dist/KFoldCrossValidation";

export class InterpolatedSmoothing<Symbol> extends TrainedSmoothing<Symbol>{

    private lambda1: number
    private lambda2: number
    private simpleSmoothing: SimpleSmoothing<Symbol>

    /**
     * Constructor of {@link InterpolatedSmoothing}
     * @param simpleSmoothing smoothing method.
     */
    constructor(simpleSmoothing: SimpleSmoothing<Symbol> = new GoodTuringSmoothing()) {
        super();
     ...
     */
    private learnBestLambda(nGrams: Array<NGram<Symbol>>, kFoldCrossValidation: KFoldCrossValidation<Array<Symbol>>,
                            lowerBound: number): number {
        let bestPrevious = -1, upperBound = 0.999, bestLambda = (lowerBound + upperBound) / 2;
        let numberOfParts = 5;
        let testFolds = new Array<Array<Array<Symbol>>>();
        for (let i = 0; i < 10; i++){
     ...
     */
    private learnBestLambdas(nGrams: Array<NGram<Symbol>>, kFoldCrossValidation: KFoldCrossValidation<Array<Symbol>>,
                             lowerBound1: number, lowerBound2: number): Array<number>{
        let upperBound1 = 0.999, upperBound2 = 0.999, bestPrevious = -1, bestLambda1 = (lowerBound1 + upperBound1) / 2,
